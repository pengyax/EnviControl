...
def complaints_envic(path,vendor_list,year_list = [2022,2023],div_list = [21,22,51]):
    try:
        df_complaints_detail = pd.read_excel(path,sheet_name=0)
    ...
    & (df_complaints_detail['Division'].isin(div_list))
...
def inspection_envic(path,vendor_list,year_list = [2022,2023]):
    try:
        ...
        df_inspection_detail_query = (df_inspection_detail['Vendor'].isin(vendor_list)) & (df_inspection_detail['Year'].isin(year_list)) & (df_inspection_detail['Path'] == 'QIM') \
...
if __name__ == "__main__":
    ...
    df_vendor_list = pd.read_excel('./datasource/vendor_list.xlsx',sheet_name=0)
    ...
    df_purchasing_2022 = pd.read_excel('./datasource/2022 Purchasing.xlsx',sheet_name=0)
    df_purchasing = pd.concat([df_purchasing_2023,df_purchasing_2022],ignore_index=True)
    df_purchasing = df_purchasing.loc[df_purchasing['Vendor Name'].isin(vendor_list)]
    print(len(df_purchasing))
    
    complaints_path = r'./datasource/US Complaint Data 201901-202301.xlsx'
    df_complaints_foreignParticulate = complaints_envic(complaints_path,vendor_list)
    print(df_complaints_foreignParticulate)
    
    insepction_path = r'./datasource/InspectionDatabase_duplicateCombine_2022&2023.xlsx'
    ...
    with pd.ExcelWriter('EnviControl1.xlsx') as writer:
    ...
    df = df.loc[~((df['Year'] == 2023) & (df['Month'] >=2))]
    ...
    with pd.ExcelWriter('EnviControl_gy_011.xlsx') as writer:
        df_vendor_list.to_excel(writer,index=False,sheet_name='vendor_list')
